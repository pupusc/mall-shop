...
public class HTMLEscapeUtil {
    ...
    public static String cleanStyle(String html) {
        Pattern p = ValidationPattern.getValidationPattern("HTMLStyle");

        Document doc = Jsoup.parse(html);
        Elements elements = doc.select("[" + CSS_ATTRIBUTE_STYLE_NAME + "]");
        for (Element e : elements) {
            String styleValue = e.attr(CSS_ATTRIBUTE_STYLE_NAME);
            if (!p.matcher(styleValue).matches()) {
                e.removeAttr(CSS_ATTRIBUTE_STYLE_NAME);     // 异常内容则移除样式
            }

            String[] ss = styleValue.split(";");
            for (String s : ss) {
                String[] sk = StringUtils.trim(s).split(":");

                if (sk.length < 2) {
                    e.removeAttr(CSS_ATTRIBUTE_STYLE_NAME); // 错误属性则移除样式
                    continue;
                }

                if (!ArrayUtils.contains(QmWhitelist.cssStyle(), sk[0].toLowerCase())) {
                    e.removeAttr(CSS_ATTRIBUTE_STYLE_NAME); // 异常属性则移除样式
                    continue;
                }

                if (StringUtils.indexOfAny(sk[1].toLowerCase(), "expression", "javascript") > 0) {
                    e.removeAttr(CSS_ATTRIBUTE_STYLE_NAME); // 恶意关键词则移除样式
                    continue;
                }
    ...
    private static synchronized Map<Character, String> mkCharacterToEntityMap() {
        ...
        map.put((char) 39, "&#39;");	/* quotation mark */
