...
public interface WxLiveAssistantGoodsRepository extends JpaRepository<WxLiveAssistantGoodsModel, Long>, JpaSpecificationExecutor<WxLiveAssistantGoodsModel> {
     ...
     * 查找商品是否存在于还未开播的直播计划中
     */
    @Query(value = "select lg.* from t_wx_live_assistant as l join t_wx_live_assistant_goods as lg on lg.assist_id=l.id where lg.goods_id in ?1 and lg.del_flag=0 and now() <= l.end_time", nativeQuery = true)
