...
@Component
public class WaitPayTailBuilder implements Builder {
    ...
    public FlowState supportState() {
        return FlowState.WAIT_PAY_TAIL;
    }
    ...
    public StateMachine<FlowState, TradeEvent> build(Trade trade, BeanFactory beanFactory) throws Exception {
        ...
        builder.configureConfiguration()
                .withConfiguration()
                .machineId("WaitPayTail").beanFactory(beanFactory)
                .listener(listener())
        ;

        builder.configureStates()
                .withStates()
                .initial(FlowState.WAIT_PAY_TAIL)
        ...
        builder.configureTransitions()
                ...
                // 待支付尾款 -> [审核驳回] -> 作废
                .withExternal()
                .source(FlowState.WAIT_PAY_TAIL).target(FlowState.VOID)
