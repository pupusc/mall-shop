...
@Component
public class WaitPayEarnestBuilder implements Builder {
    ...
    public FlowState supportState() {
        return FlowState.WAIT_PAY_EARNEST;
    }
    ...
    public StateMachine<FlowState, TradeEvent> build(Trade trade, BeanFactory beanFactory) throws Exception {
        ...
        builder.configureConfiguration()
                .withConfiguration()
                .machineId("WaitPayEarnest").beanFactory(beanFactory)
                .listener(listener())
        ;

        builder.configureStates()
                .withStates()
                .initial(FlowState.WAIT_PAY_EARNEST)
        ...
        builder.configureTransitions()
                ...
                // 待支付定金 -> [审核驳回] -> 作废
                .withExternal()
                .source(FlowState.WAIT_PAY_EARNEST).target(FlowState.VOID)
