...
@Service
@Slf4j
public class GoodsEvaluateSql extends SqlTool {
    ...
    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY customer_id) a JOIN goods_evaluate b ON " +
        ...
        if (isFirst){
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "create_time) as time_str FROM goods_evaluate b WHERE b.create_person != 'system' and b.del_flag " +
                        "= 0 and " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "lastTime from " +
                        "goods_evaluate b where " +
                        "b.create_person != 'system' and b.del_flag = 0 and " + queryTimeStr;
            } else {
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from goods_evaluate b where " +
                        "b.create_person != 'system' and b.del_flag = 0 and " + queryTimeStr;
            }
    ...
    private String preferenceSelectSqlStr(boolean isBigData){
        String sql = "SELECT customer_id, count(%s) as total, %s as result, max(b.create_time) as last_time FROM goods_evaluate b where b" +
                ".create_person " +
                "!= 'system' and b.del_flag = 0 and ";
        if (isBigData){
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.GOODS_EVALUATE};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo){
        ...
        log.info("DimensionName.GOODS_EVALUATE SQL:{}", sql);
        return sql;
    }

    public static void main(String[] args) {
        GoodsEvaluateSql sql = new GoodsEvaluateSql();
        StatisticsTagInfo info = new StatisticsTagInfo();
        info.setTagId(1L);
        info.setTagName("评价商品标签值标签");
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.GOODS_EVALUATE);
...
//        paramInfo.setDataRange(Collections.singletonList(vo));
        ...
        paramInfo4.setParamValue("1,2,3,4");
        List<StatisticsTagParamInfo> s = new ArrayList<>();
        s.add(paramInfo1);s.add(paramInfo2);
//        s.add(paramInfo3);
        s.add(paramInfo4);
