 ...
 * @description: 加购
 * @create: 2020-08-18 15:26
 **/
@Service
@Slf4j
public class AddCarSql extends SqlTool {
    ...
    private final Map<TagParamColumn,String> PREFERENCE_RESULT_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 1819263693130501269L;
        {
            ...
            put(TagParamColumn.CATE_TOP_ID,"cate_top_id");
            put(TagParamColumn.CATE_ID,"cate_id");
            put(TagParamColumn.BRAND_ID,"brand_id");
            put(TagParamColumn.GOODS_ID,"goods_id");
            put(TagParamColumn.STORE_ID,"store_id");
        }
    };

    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY customer_id) a JOIN purchase_action b ON " +
        ...
        if (isFirst){ // 首末查询
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "create_time) as time_str FROM purchase_action b WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "lastTime from purchase_action b where " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from purchase_action b where " + queryTimeStr;
            }
        }
    ...
    private String preferenceFirstSqlStr(boolean isBigData){
        ...
        if (isBigData){
            sql = "SELECT %d AS tagId, '%s' as tagName, '%s' as tagType, %d as type, resAll.customer_id, " +
                    ...
                    "() over (partition by res.customer_id order by res.total desc, res.last_time desc ) rankNum, res.* " +
                    "FROM ( ";
        }
        return sql;
    }
    ...
    private String notFirstLast(StatisticsTagInfo tagInfo){
        ...
        // 除了综合标签，dimensionInfoList只会有1个元素
        StatisticsDimensionInfo info = tagInfo.getDimensionInfoList().get(0);
        String sql = getFrontSql(false, tagInfo).concat(" ");

        // where条件
        StringBuilder where = new StringBuilder();
        if (RelationType.AND.equals(info.getRelationType())){ // 条件之间and关系
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.ADD_CAR};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo){
        ...
        log.info("DimensionName.ADD_CAR SQL:{}", sql);
        return sql;
    }

    public static void main(String[] args) {
        AddCarSql sql = new AddCarSql();
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.ADD_CAR);
...
//        paramInfo.setParamValue("14:00-15:00,18:00-19:00");
        ...
        paramInfo3.setParamName(TagParamColumn.CATE_TOP_ID);
        paramInfo3.setParamValue("1,2");
        List<StatisticsTagParamInfo> s = new ArrayList<>();
        s.add(paramInfo1);
        s.add(paramInfo2);
        s.add(paramInfo3);
        statisticsDimensionInfo.setParamInfoList(s);
//        statisticsDimensionInfo.setParamInfoList(new ArrayList<>());
