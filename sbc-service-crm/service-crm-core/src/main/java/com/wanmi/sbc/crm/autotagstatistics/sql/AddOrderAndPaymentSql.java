 ...
 * @description: 下单、付款
 * @create: 2020-08-19 20:37
 **/
@Service
@Slf4j
public class AddOrderAndPaymentSql extends SqlTool {
    private final Map<TagParamColumn,String> RESULT_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        ...
        {
            put(TagParamColumn.TERMINAL_SOURCE,"a.order_source");
    ...
    private final Map<TagParamColumn,String> FRONT_RESULT_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 8663638743504508858L;
        {
            put(TagParamColumn.TERMINAL_SOURCE,"y.order_source");
    ...
    private final Map<TagParamColumn,String> FRONT_FIRST_RESULT_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 8663638743504508858L;
        {
            put(TagParamColumn.TERMINAL_SOURCE,"t.order_source");
    ...
    private final Map<TagParamColumn,String> QUOTA_FRONT_FIRST_RESULT_COLUMN_MAP_COMMON = new HashMap<TagParamColumn,
            String>(){
        {
            put(TagParamColumn.TERMINAL_SOURCE,"h.order_source");
    ...
    private final Map<TagParamColumn,String> GROUP_BY_MAP = new HashMap<TagParamColumn,String>(){
        ...
        {
            put(TagParamColumn.TERMINAL_SOURCE, "GROUP BY a.customer_id, a.order_source ");
            ...
            put(TagParamColumn.STORE_ID,"GROUP BY a.customer_id, b.store_id ");
        }
    };
    ...
    private final Map<TagParamColumn,String> WHERE_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        ...
        {
            put(TagParamColumn.TERMINAL_SOURCE," a.order_source in (%s) ");
    ...
    private final Map<TagParamColumn,String> WHERE_NO_NULL_MAP = new HashMap<TagParamColumn,String>(){
        ...
        {
            put(TagParamColumn.TERMINAL_SOURCE," and a.order_source is not null ");
            put(TagParamColumn.MONEY, " and a.total_price is not null ");
    ...
    private final Map<TagParamColumn,String> HAVING_MAP = new HashMap<TagParamColumn,String>(){
        ...
        {
            put(TagParamColumn.TERMINAL_SOURCE," y.order_source in (%s) ");
    ...
    private String getFrontSql(boolean isFirst, StatisticsTagInfo tagInfo){
        ...
        DimensionName dimensionName = info.getDimensionName();
        String frontSql = "";
        String date = "";
        String queryTimeStr =
        ...
        String join = " LEFT JOIN trade_item b on a.tid = b.tid ";
        ...
        if (isFirst){
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + ", " + tagInfo.getTagType().toValue() + " as " +
                        ...
                        ".create_time) as time_str FROM trade a " + join + " WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + ", " + tagInfo.getTagType().toValue() + " as " +
                        "tagType, a.customer_id" + date + " FROM (SELECT a.customer_id," + maxOrMin + "(DISTINCT a" +
                        ".create_time) as " +
                        "time_str FROM trade a " + join + " WHERE " + queryTimeStr;
            }
        } else {
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "lastTime  from trade a " + join + " WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select a.customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from trade a " + join + " " +
                        "WHERE " +  queryTimeStr;
            }
        }
        if (DimensionName.PAY_ORDER.equals(dimensionName)){
            frontSql =  frontSql.concat(" and a.pay_state = 2 ");
        }
    ...
    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY a.customer_id ) e JOIN trade a on e" +
            ".customer_id = a.customer_id " +
            "AND e.time_str = a.create_time ";

    private static final String FIRST_OR_LAST_JOIN_SQL_1 = "JOIN trade_item b on a.tid = b.tid ";
    ...
    private String preferenceSelectSqlStr(boolean isBigData, StatisticsDimensionInfo info){
        String sql = "SELECT a.customer_id, count(%s) as total, %s as result, max(a.create_time) as last_time " +
                "FROM trade a LEFT JOIN trade_item b on a.tid = b.tid where ";
        if (TagParamColumn.MONEY.equals(info.getParamResult().getParamName())
                || TagParamColumn.NUM.equals(info.getParamResult().getParamName())){
            ...
            if (TagParamColumn.MONEY.equals(info.getParamResult().getParamName())) {
                ...
                if (paras.length == 1){
                    ...
                    for (int i = 1; i < paras.length; i++){
                        ...
                        String rangWhere = String.format(PREFERENCE_RANG_MONEY_WHERE_STR, item1, item2);
                        if (i == 1){
                ...
                for (int i = 0; i < maps.size(); i++) {
                    ...
                    String rangWhere = String.format(PREFERENCE_RANG_TIME_WHERE_STR, item1,
                            item2);
                    if (i == 0) {
                        sql = sqlItem.concat(rangWhere).concat(PREFERENCE_TIME_AND_MONEY_GROUP_SQL_STR);
                    } else {
    ...
    private String firstLase(StatisticsTagInfo tagInfo){
        ...
        List<TagParamColumn> tagParamColumns = Arrays.asList(TagParamColumn.CATE_ID, TagParamColumn.BRAND_ID,
                TagParamColumn.GOODS_ID, TagParamColumn.STORE_ID, TagParamColumn.CATE_TOP_ID);
        ...
        if (TagType.MULTIPLE.equals(tagInfo.getTagType()) || TagType.RANGE.equals(tagInfo.getTagType())){
            ...
            if (DimensionName.PAY_ORDER.equals(info.getDimensionName())){
                where =  where.concat(" and a.pay_state = 2 ");
            }
            sql = sql.concat(" where 1 = 1 ").concat(where).concat(notNull)
                    .concat(") h group by h.customer_id,"+
                            QUOTA_FRONT_FIRST_RESULT_COLUMN_MAP_COMMON.get(info.getParamResult().getParamName()) +
                            ") h ) t ");
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.ADD_ORDER, DimensionName.PAY_ORDER};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo) {
        ...
        log.info("PAY_ORDER SQL:{}", sql);
        return sql;
    }


    public static void main(String[] args) {
        AddOrderAndPaymentSql sql = new AddOrderAndPaymentSql();
        StatisticsTagInfo info = new StatisticsTagInfo();
        info.setTagId(1L);
        info.setTagName("下单标签值标签");
        info.setTagType(TagType.RANGE);
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.PAY_ORDER);
