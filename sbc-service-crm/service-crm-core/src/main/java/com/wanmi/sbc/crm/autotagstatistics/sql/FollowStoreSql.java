...
@Service
@Slf4j
public class FollowStoreSql extends SqlTool {
    ...
    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY customer_id) a JOIN store_customer_follow_action b ON " +
        ...
        if (isFirst){
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "create_time) as time_str FROM store_customer_follow_action b WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "lastTime from " +
                        "store_customer_follow_action b where " + queryTimeStr;
            } else {
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from store_customer_follow_action b where " + queryTimeStr;
            }
    ...
    private String preferenceSelectSqlStr(boolean isBigData){
        String sql = "SELECT customer_id, count(%s) as total, %s as result, max(b.create_time) as last_time FROM store_customer_follow_action b " +
                "where ";
        if (isBigData){
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.FOLLOW_STORE};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo){
        ...
        log.info("DimensionName.FOLLOW_STORE SQL:{}", sql);
        return sql;
    }

    public static void main(String[] args) {
        FollowStoreSql sql = new FollowStoreSql();
        StatisticsTagInfo info = new StatisticsTagInfo();
        info.setTagId(1L);
        info.setTagName("关注店铺标签值标签");
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.FOLLOW_STORE);
        ...
        paramInfo.setParamName(TagParamColumn.TO_DATE_NUM);
