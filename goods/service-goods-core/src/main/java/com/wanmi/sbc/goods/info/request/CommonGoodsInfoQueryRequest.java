...
@AllArgsConstructor
@NoArgsConstructor
public class CommonGoodsInfoQueryRequest extends BaseQueryRequest {
    ...
    public Specification<GoodsInfo> getWhereCriteria() {
        return (root, cquery, cbuild) -> {
             ...
             * 企业购商品的审核状态
             */
            if (enterPriseAuditState != null) {
                predicates.add(cbuild.equal(root.get("enterPriseAuditState"), enterPriseAuditState.toValue()));
            } else {
                predicates.add(cbuild.or(root.get("enterPriseAuditState").isNull(), cbuild.equal(root.get("enterPriseAuditState"), EnterpriseAuditState.INIT.toValue())));
            }

            if (enterpriseFlag != null && enterpriseFlag) {
                predicates.add(cbuild.notEqual(root.get("goodsType"), 3));
            }
