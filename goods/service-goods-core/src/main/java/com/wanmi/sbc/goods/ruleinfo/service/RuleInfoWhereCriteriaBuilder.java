 ...
 */
public class RuleInfoWhereCriteriaBuilder {
    public static Specification<RuleInfo> build(RuleInfoQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            ...
            // 模糊查询 - 规则说明
            if (StringUtils.isNotEmpty(queryRequest.getRuleContent())) {
                predicates.add(cbuild.like(root.get("ruleContent"), StringUtil.SQL_LIKE_CHAR
                        .concat(XssUtils.replaceLikeWildcard(queryRequest.getRuleContent()))
                        .concat(StringUtil.SQL_LIKE_CHAR)));
            }

            // 规则类型 0:预约 1:预售
            if (queryRequest.getRuleType() != null) {
                predicates.add(cbuild.equal(root.get("ruleType"), queryRequest.getRuleType()));
            }
