...
//    @Builder.Default
     ...
     * 筛选第三方平台订单
     */
    @ApiModelProperty(value = "筛选第三方平台订单")
     ...
     * 是否查询有赞的历史订单
     */
    private Boolean yzOrderFlag;


    /**
     * 是否周期购订单   发货日历配送状态
     */
    private CycleDeliverStatus deliverStatus;
    ...
    private List<Criteria> getCommonCriteria() {
        ...
        if(Objects.nonNull(providerName)){
            criterias.add(Criteria.where("supplier.supplierName").is(providerName));
        }

        if(Objects.nonNull(providerCode)){
            criterias.add(Criteria.where("supplier.supplierCode").is(providerCode));
        }
        ...
        if (StringUtils.isNotBlank(supplierCode)) {
            criterias.add(Criteria.where("supplierCode").is(supplierCode));
        }

        if (StringUtils.isNotBlank(supplierName)) {
            criterias.add(Criteria.where("supplierName").is(supplierName));
        }
...
//            criterias.add(new Criteria().orOperator(andCriteria,Criteria.where("storeBagsFlag").is(DefaultFlag.YES)));
        ...
        // 是否周期购
        if (Objects.nonNull(cycleBuyFlag) && cycleBuyFlag) {
            criterias.add(Criteria.where("cycleBuyFlag").is(cycleBuyFlag));
            //周期购发货日历状态
            if (Objects.nonNull(deliverStatus)) {
                criterias.add(Criteria.where("tradeCycleBuyInfo.deliverCalendar.cycleDeliverStatus").is(deliverStatus));
            }
        }

        // 过滤有赞历史订单
        if (Objects.nonNull(yzOrderFlag) && !yzOrderFlag) {
            criterias.add(Criteria.where("yzOrderFlag").is(yzOrderFlag));
        }
