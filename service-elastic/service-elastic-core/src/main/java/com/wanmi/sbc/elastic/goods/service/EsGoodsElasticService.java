...
//        }
        ...
        logger.info("EsGoodsElasticService initEsGoods condition:{} total count is :{}", JSON.toJSONString(goodsCountQueryRequest), totalCount);
        if (totalCount <= 0) {
            return;
        }

        logger.info("##################商品同步到ES服务器中 begin##########################");
         ...
         */
        Integer pageSize = request.getPageSize() == null ? 2000 : request.getPageSize();
...
//                        }
                        ...
                        finalGoodsInfos.stream().filter(goodsInfoVO -> goods.getGoodsId().equals(goodsInfoVO.getGoodsId()))
                                .forEach(goodsInfoVO -> {
                                    ...
                                    esGoodsInfo.setGoodsChannelTypeList(esGoods.getGoodsChannelTypeList());
