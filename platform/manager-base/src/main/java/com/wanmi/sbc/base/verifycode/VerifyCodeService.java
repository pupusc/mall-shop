...
public class VerifyCodeService {
     ...
     * @param phone 手机号
     */
    public void addVerifyCodeErrorCount(String phone) {
        String count = stringRedisTemplate.opsForValue().get("verifyCodeError_" + phone);
        if (count != null) {
            //如果输错5次
            if(Long.parseLong(count) >= 5) {
                //移除短信验证码
                stringRedisTemplate.delete(String.format("%s_%s", "S2B_BOSS_CHANGE_PASSWORD", phone));
                stringRedisTemplate.delete(String.format("%s_%s", "SUPPLIER_CHANGE_PASSWORD", phone));
            } else {
                //增加短信验证码输入错误次数
                stringRedisTemplate.opsForValue().set("verifyCodeError_" + phone, String.valueOf((Long.parseLong(count) + 1)), 10,
                        TimeUnit.MINUTES);
            }
        } else {
            //增加短信验证码输入错误次数
            stringRedisTemplate.opsForValue().set("verifyCodeError_" + phone, "1", 10,
                    TimeUnit.MINUTES);
        }
    }
    ...
    public boolean validateSmsVerifyCodeAgain(String phone, String verifyCode, VerifyType verifyType) {
        String cacheCode = stringRedisTemplate.opsForValue().get(String.format("%s_%s", verifyType, phone));
        if(cacheCode == null) {
            throw new SbcRuntimeException(CommonErrorCode.VERIFICATION_CODE_FAILURE);
        }
        if (cacheCode.equals(verifyCode)) {
            return true;
        } else {
