...
//    }
    ...
    public BaseResponse listCouponByCustomer(@RequestBody CouponByCustomerReq couponByCustomerReq) {
        ...
        for (CouponCodeVO couponCodeVO : couponCodeList) {
            ...
            couponVO.setDenomination(couponCodeVO.getDenomination());
            couponVO.setLimitPrice(FullBuyType.FULL_MONEY.equals(couponCodeVO.getFullBuyType()) ? couponCodeVO.getFullBuyPrice() : BigDecimal.ZERO);
            couponVO.setLimitScope(couponCodeVO.getScopeType().toValue());
            couponVO.setHasFetch(true);
            couponVO.setNearOverdue(couponCodeVO.isNearOverdue());
            couponVO.setStatus(couponCodeVO.getStatus().toValue());
            result.add(couponVO);
        }
        return BaseResponse.success(result);
    }
