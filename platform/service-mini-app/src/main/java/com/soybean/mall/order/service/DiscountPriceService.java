...
//
//    //copy原来凑单页面
//    public TradeConfirmResp test (DiscountPriceReq discountPriceReq) {
//
...
//        tradeItemGroupVO.setTradeItems(Lists.newArrayList());
//        tradeItemGroupVO.setSupplier(new SupplierVO());
//        tradeItemGroupVO.setTradeMarketingList(Lists.newArrayList());
//
...
//            //周期购使用单品运费
//            DefaultFlag freightTemplateType = Objects.nonNull(g.getCycleBuyInfo()) ? DefaultFlag.YES : storeMap.get(g.getSupplier().getStoreId()).getFreightTemplateType();
...
////            skuResp.getGoodsInfos().forEach(item -> {
////                //企业购商品
////                if (DistributionGoodsAudit.CHECKED.equals(item.getDistributionGoodsAudit()) || DefaultFlag.YES.equals(g.getStoreBagsFlag()) || Objects.nonNull(g.getGrouponForm())) {
////                    item.setCount(null);
...
//                    new VerifyGoodsRequest(tradeItemDTOList, Collections.emptyList(), KsBeanUtil.convert(skuResp, TradeGoodsInfoPageDTO.class), g.getSupplier().getStoreId(), true);
...
//            TradeConfirmResp response, List<TradeItemGroupVO> tradeItemGroups, String customerId) {
//        TradeItemGroupVO tradeItemGroup = tradeItemGroups.get(NumberUtils.INTEGER_ZERO);
//        TradeConfirmItemVO confirmItem = response.getTradeConfirmItems().get(NumberUtils.INTEGER_ZERO);
//        // 组合购标记
