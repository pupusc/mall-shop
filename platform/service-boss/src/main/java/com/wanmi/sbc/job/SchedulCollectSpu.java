...
public class SchedulCollectSpu {     //订单同步
    ...
    @Autowired
    private StockAppointmentJobHandler appointmentSaleActivityJobHandler;


    //@Scheduled(cron = "0/30 * * * * ?")        //0/30 * * * * ?   表示每5秒 执行任务
    @Scheduled(cron = "0 03 12  ? * *")          //0 30 23 ? * *   每天23:30触发
    ...
    public void doJob(){
        ...
        try {
            appointmentSaleActivityJobHandler.execute(null);
        } catch (Exception e) {
            e.printStackTrace();
