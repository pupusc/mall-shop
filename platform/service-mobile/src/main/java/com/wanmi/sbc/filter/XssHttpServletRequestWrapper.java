...
public class XssHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public XssHttpServletRequestWrapper(HttpServletRequest servletRequest, String[] excludeFields, Integer pns, Integer pvs) {
        ...
        try {
            requestBody = StreamUtils.copyToByteArray(servletRequest.getInputStream());
            logger.info("XssHttpServletRequestWrapper requestURI:{} requestBody {}", servletRequest.getRequestURI(), new String(requestBody, "UTF-8"));
        } catch (IOException e) {
            logger.error("XssHttpServletRequestWrapper " , e);
        }
    }
        ...
        while (en.hasMoreElements()) {
            try {
                ...
                if(!ArrayUtils.contains(excludeFields, name)){
                    clean = ValidationPattern.getValidInput("HTTP header value (" + name + "): " + value, value, "HTTPHeaderValue", 300, true);
                }
