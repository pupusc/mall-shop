...
@Builder
@ApiModel
public class ThirdPlatformTradeQueryDTO extends BaseQueryRequest {

    private static final long serialVersionUID = -2971259121898549718L;
    /**
     * 订单编号
     */
     ...
     * 批量非支付状态
     */
    @ApiModelProperty(value = "批量非支付状态")
    private List<PayState> notPayStates;
    ...
    private List<Criteria> getCommonCriteria() {
        ...
        if (StringUtils.isNoneBlank(tradeId)) {
            criterias.add(XssUtils.regex("tradeId", tradeId));
        }
        ...
        //批量非支付状态
        if (CollectionUtils.isNotEmpty(notPayStates)) {
            criterias.add(Criteria.where("tradeState.payState").nin(notPayStates.stream().map(PayState::getStateId).collect(Collectors.toList())));
        }
