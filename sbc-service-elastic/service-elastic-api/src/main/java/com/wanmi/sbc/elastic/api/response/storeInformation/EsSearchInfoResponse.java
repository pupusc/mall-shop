...
@Data
@ApiModel
public class EsSearchInfoResponse<T> implements Serializable {

    private static final long serialVersionUID = 8963855907113740313L;

    public static final Logger LOGGER = LoggerFactory.getLogger(EsSearchInfoResponse.class);
     ...
     * @return
     */
    public EsSearchInfoResponse addTotalNum(SearchResponse response) {
     ...
     * @return
     */
    private EsSearchInfoResponse addQueryResults(SearchResponse response, ResultsMapper resultsMapper, Class<T> clazz) {
        if (Objects.nonNull(resultsMapper)) {
            data = resultsMapper.mapResults(response, clazz, null).getContent();
        }
     ...
     * @return
     */
    public static <T> EsSearchInfoResponse<T> build(SearchResponse searchResponse, ResultsMapper resultsMapper,
                                                    Class<T> clazz) {
        return new EsSearchInfoResponse().addQueryResults(searchResponse, resultsMapper, clazz).addTotalNum(searchResponse);
    }
