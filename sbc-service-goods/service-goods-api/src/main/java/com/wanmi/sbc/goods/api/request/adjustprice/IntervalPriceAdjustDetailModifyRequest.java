...
@NoArgsConstructor
@Builder
public class IntervalPriceAdjustDetailModifyRequest extends GoodsBaseRequest {

    private static final long serialVersionUID = 9081956796614730960L;
    /**
     * 调价单号
     */
    ...
    private List<GoodsIntervalPriceDTO> intervalPriceList;

    @Override
    public void checkParam() {
        BigDecimal marketingPrice = this.marketingPrice;
        Validate.isTrue(Objects.nonNull(marketingPrice) || CollectionUtils.isNotEmpty(intervalPriceList), NULL_EX_MESSAGE,
                "marketPrice & levelPriceList");
        checkPrice(marketingPrice, "市场价");
        intervalPriceList.forEach(i -> {
            if (Objects.nonNull(i.getPrice())) {
                checkPrice(i.getPrice(), "区间价");
            }
        });
    }
