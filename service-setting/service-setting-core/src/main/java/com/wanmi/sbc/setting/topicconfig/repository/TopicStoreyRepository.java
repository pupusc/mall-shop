...
public interface TopicStoreyRepository extends JpaRepository<TopicStorey, Integer>,
    ...
    List<TopicStorey> getAvailByTopicId(Integer topicId);

    @Modifying
    @Query("update TopicStorey w set w.status = ?2, w.updateTime = now() where w.id = ?1")
    int enable(Integer storeyId,Integer status);

    @Query("from TopicStorey w where w.topicId = ?1 and w.deleted = 0 order by w.sorting asc ")
    List<TopicStorey> getByTopicId(Integer topicId);

    @Modifying
    @Query("update TopicStorey w set w.deleted = 1, w.updateTime = now() where w.id = ?1")
    int delete(Integer storeyId);

    @Query("from TopicStorey w where w.storeyType = ?1 and w.deleted = 0 and w.status = 1")
    List<TopicStorey> getAvailByTopicType(Integer storeyType);
