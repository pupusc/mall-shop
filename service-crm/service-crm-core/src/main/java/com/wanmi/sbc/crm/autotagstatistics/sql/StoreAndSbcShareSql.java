...
@Service
@Slf4j
public class StoreAndSbcShareSql extends SqlTool {
    ...
    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY customer_id) a JOIN store_share_record b ON " +
    ...
    private String getFrontSql(boolean isFirst, StatisticsTagInfo tagInfo){
        ...
        // 分享商城、分享店铺
        String indextype = "";
        if (DimensionName.SHARE_S2B.equals(info.getDimensionName())){
            indextype = " and b.index_type = 0 ";
        } else if (DimensionName.SHARE_STORE.equals(info.getDimensionName())){
            indextype = " and b.index_type = 1 ";
        }
        ...
        if (isFirst){
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "create_time) as time_str FROM store_share_record b WHERE " + queryTimeStr + indextype;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "lastTime from " +
                        "store_share_record b where "
                        + queryTimeStr + indextype;
            } else {
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from store_share_record b where "
                        + queryTimeStr + indextype;
            }
        }
    ...
    private String preferenceSelectSqlStr(boolean isBigData){
        String sql = "SELECT customer_id, count(%s) as total, %s as result, max(b.create_time) as last_time FROM store_share_record b where ";
        if (isBigData){
    ...
    private String preference (StatisticsTagInfo tagInfo){
        ...
        String sqlItem = sql.concat(indextype).concat(where.toString());
        if (TagParamColumn.TIME.equals(info.getParamResult().getParamName())){
            // 时间范围和金额范围特殊处理
            List<RangeParamVo> rangeParamVos = info.getParamResult().getDataRange();
            List<Map<String, String>> maps = rangeParamVos.get(0).getDataValue();
    ...
    private String firstLase(StatisticsTagInfo tagInfo){
        ...
        // 分享商城、分享店铺
        if (DimensionName.SHARE_S2B.equals(info.getDimensionName())){
            where = where.concat(" and b.index_type = 0 ");
        } else if (DimensionName.SHARE_STORE.equals(info.getDimensionName())){
            where = where.concat(" and b.index_type = 1 ");
        }
        // 指标值范围
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.SHARE_STORE, DimensionName.SHARE_S2B};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo){
        ...
        log.info("DimensionName.SHARE_S2B SQL:{}", sql);
        return sql;
    }

    public static void main(String[] args) {
        StoreAndSbcShareSql sql = new StoreAndSbcShareSql();
        StatisticsTagInfo info = new StatisticsTagInfo();
        info.setTagId(1L);
        info.setTagName("分享商城、店铺标签值标签");
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.SHARE_S2B);
