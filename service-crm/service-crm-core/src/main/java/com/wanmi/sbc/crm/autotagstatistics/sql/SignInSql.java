...
@Service
@Slf4j
public class SignInSql extends SqlTool {
    ...
    private final Map<TagParamColumn,String> QUOTA_FRONT_RESULT_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 8663638743504508858L;
        {
            put(TagParamColumn.TERMINAL_SOURCE,"y.terminal_source");
            put(TagParamColumn.NUM,"y.total");
            put(TagParamColumn.MONEY,"y.goods_price");
            put(TagParamColumn.DAY_NUM,"y.total");
            put(TagParamColumn.TO_DATE_NUM,"y.days");
            put(TagParamColumn.TIME,"y.create_time");
            put(TagParamColumn.DATE,"y.days");
        ...
        if (isFirst){
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "create_time) as time_str FROM customer_sign_record_action b WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "select customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + ",max( b.create_time ) as " +
                        "lastTime from " +
                        "customer_sign_record_action b where " + queryTimeStr;
            } else {
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from customer_sign_record_action b where " + queryTimeStr;
            }
        }
    ...
    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY customer_id) a JOIN customer_sign_record_action b ON " +
    ...
    private String preferenceSelectSqlStr(boolean isBigData){
        String sql = "SELECT a.customer_id, count(%s) as total, %s as result, max(a.create_time) as last_time FROM " +
                "customer_sign_record_action a " +
                "LEFT " +
                "JOIN (SELECT customer_id, count(*) as total, count(DISTINCT DATE (create_time)) as day_total FROM " +
                "customer_sign_record_action GROUP BY customer_id) b on a.customer_id = b.customer_id where ";
        if (isBigData){
    ...
    private static final String PREFERENCE_GROUP_SQL_STR = "GROUP BY a.customer_id, %s ORDER BY " +
            "a.customer_id ";

    private static final String PREFERENCE_RANG_WHERE_STR = "AND date_format(create_time,'HH:mm') >= '%s' AND date_format(create_time,'HH:mm') " +
            "< '%s' ";
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.SIGN_IN};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo){
        ...
        log.info("DimensionName.SIGN_IN SQL:{}", sql);
        return sql;
    }
    ...
    public static void main(String[] args) {
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.SIGN_IN);
...
//        statisticsDimensionInfo.setDimensionType(TagDimensionFirstLastType.NO_FIRST_LAST);
        ...
        paramInfo.setParamName(TagParamColumn.DATE);
...
//        paramInfo4.setParamName(TagParamColumn.NUM);
//        paramInfo4.setParamValue("<10");
//        StatisticsTagParamInfo paramInfo5 = new StatisticsTagParamInfo();
//        paramInfo5.setParamName(TagParamColumn.DAY_NUM);
//        paramInfo5.setParamValue(">20");
