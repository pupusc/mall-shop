...
@Service
@Slf4j
public class ShareGoodsSql extends SqlTool {
    private final Map<TagParamColumn,String> RESULT_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 8663638743504508858L;
        {
            ...
            put(TagParamColumn.CATE_TOP_ID,"a.cate_top_id");
            put(TagParamColumn.CATE_ID,"a.cate_id");
            put(TagParamColumn.BRAND_ID,"a.brand_id");
            put(TagParamColumn.GOODS_ID,"a.goods_id");
    ...
    private final Map<TagParamColumn,String> GROUP_BY_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 8076938296165644926L;
        {
            ...
            put(TagParamColumn.CATE_TOP_ID,"GROUP BY a.customer_id, a.cate_top_id ");
            put(TagParamColumn.CATE_ID,"GROUP BY a.customer_id, a.cate_id ");
            put(TagParamColumn.BRAND_ID,"GROUP BY a.customer_id, a.brand_id ");
            put(TagParamColumn.GOODS_ID,"GROUP BY a.customer_id, a.goods_id ");
    ...
    private final Map<TagParamColumn,String> WHERE_COLUMN_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 4509761025924962579L;
        {
            ...
            put(TagParamColumn.CATE_TOP_ID," a.cate_top_id in (%s) ");
            put(TagParamColumn.CATE_ID," a.cate_id in (%s) ");
            put(TagParamColumn.BRAND_ID," a.brand_id in (%s) ");
            put(TagParamColumn.GOODS_ID," a.goods_id in (%s) ");
    ...
    private final Map<TagParamColumn,String> WHERE_NO_NULL_MAP = new HashMap<TagParamColumn,String>(){
        private static final long serialVersionUID = 7092314267873635659L;
        {
            ...
            put(TagParamColumn.CATE_TOP_ID," and a.cate_top_id is not null ");
            put(TagParamColumn.CATE_ID," and a.cate_id is not null ");
            put(TagParamColumn.BRAND_ID," and a.brand_id is not null ");
            put(TagParamColumn.GOODS_ID," and a.goods_id is not null ");
        ...
        if (isFirst){
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + ", " + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "from ( select h.customer_id, "+resultCol+",min(h.create_time) as last_time from ( select a" +
                        ...
                        ".create_time) as " +
                        "time_str FROM goods_share_record a WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + ", " + tagInfo.getTagType().toValue() + " as " +
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "lastTime from goods_share_record a WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select a.customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from goods_share_record a WHERE " + queryTimeStr;
            }
        }
    ...
    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY a.customer_id ) d JOIN goods_share_record a ON d.customer_id = a" +
            ".customer_id AND d.time_str = a.create_time ";
    ...
    private String preferenceSelectSqlStr(boolean isBigData){
        String sql = "SELECT a.customer_id, count(%s) as total, %s as result, max(a.create_time) as last_time " +
                "FROM goods_share_record a where ";
        if (isBigData){
    ...
    private static final String PREFERENCE_RANG_WHERE_STR = "AND date_format(a.create_time,'HH:mm') >= '%s' AND " +
    ...
    private void whereColumn(StatisticsTagParamInfo paramInfo, StringBuilder where, String andOr){
        ...
        if (TagParamColumn.TIME.equals(paramInfo.getParamName())){
            ...
            where.append(andOr).append(String.format(WHERE_COLUMN_MAP.get(paramInfo.getParamName()),
                    Arrays.asList(paramInfo.getParamValue().split(",")).toArray()));
        } else {
            where.append(andOr).append(String.format(WHERE_COLUMN_MAP.get(paramInfo.getParamName()),
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.SHARE_GOODS};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo){
        ...
        log.info("DimensionName.SHARE_GOODS SQL:{}", sql);
        return sql;
    }


    public static void main(String[] args) {
        ShareGoodsSql sql = new ShareGoodsSql();
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.SHARE_GOODS);
