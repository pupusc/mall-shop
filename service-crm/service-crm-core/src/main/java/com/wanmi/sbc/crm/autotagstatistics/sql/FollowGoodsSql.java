...
@Service
@Slf4j
public class FollowGoodsSql extends SqlTool {
    ...
    private static final String FIRST_OR_LAST_JOIN_SQL = "GROUP BY customer_id) a JOIN goods_customer_follow_action b ON " +
        ...
        if (isFirst){
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "create_time) as time_str FROM goods_customer_follow_action b WHERE " + queryTimeStr;
            } else {
                frontSql = "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
            ...
            if (TagType.QUOTA.equals(tagInfo.getTagType())){
                ...
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        ...
                        "lastTime from" +
                        " goods_customer_follow_action b where " + queryTimeStr;
            } else {
                frontSql =  "select " + tagInfo.getTagId() + " as tagId, " + str + "," + tagInfo.getTagType().toValue() + " as " +
                        "tagType, y.customer_id" + date + " from ( select customer_id, "
                        + RESULT_COLUMN_MAP.get(info.getParamResult().getParamName()) + " from goods_customer_follow_action b where " + queryTimeStr;
            }
    ...
    private String preferenceSelectSqlStr(boolean isBigData){
        String sql = "SELECT customer_id, count(%s) as total, %s as result, max(b.create_time) as last_time FROM " +
                "goods_customer_follow_action b where ";
        if (isBigData){
    ...
    public DimensionName[] supports() {
        return new DimensionName[] {DimensionName.FOLLOW_GOODS};
    }
    ...
    public String getSql(StatisticsTagInfo tagInfo){
        ...
        log.info("DimensionName.FOLLOW_GOODS SQL:{}", sql);
        return sql;
    }


    public static void main(String[] args) {
        FollowGoodsSql sql = new FollowGoodsSql();
        StatisticsTagInfo info = new StatisticsTagInfo();
        info.setTagId(1L);
        info.setTagName("收藏标签值标签");
        ...
        statisticsDimensionInfo.setDimensionName(DimensionName.FOLLOW_GOODS);
        ...
        paramInfo.setParamName(TagParamColumn.GOODS_ID);
...
//        s.add(paramInfo3);
        ...
        bigsql = bigsql.replaceAll("NOW\\(\\)", "CURRENT_TIMESTAMP");
        bigsql = bigsql.replaceAll("%Y-%m-%d", "yyyy-MM-dd");
        bigsql = bigsql.replaceAll("interval", "").replaceAll("day", "");
        Pattern pattern = Pattern.compile("WEEKDAY\\(.*?\\)");
        Matcher matcher = pattern.matcher(bigsql);
        while (matcher.find()){
            ...
            bigsql = bigsql.replace(weekDay, "pmod(datediff("+subStr+",'2018-01-01'),7)");
        }
